...
static void *morecore_alloc(size_t bytes, size_t *retbytes)
{
    ...
    errval_t err = paging_map_lazy(st, &buf, size, BASE_PAGE_SIZE,
                                   VREGION_FLAGS_READ_WRITE, REGION_TYPE_HEAP);

    if (err_is_fail(err)) {
        *retbytes = 0;
        DEBUG_ERR(err, "Lazy mapping in morecore_alloc failed\n");
        return NULL;
    } else {
        *retbytes = size;
        return buf;
...
static void morecore_free(void *base, size_t bytes)
{
    // AFAIK this function is never called on aarch64, see oldsys_morecore::lesscore()
