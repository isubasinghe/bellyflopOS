...
genpaddr_t cap_get_paddr(struct capref cap);
genpaddr_t cap_get_psize(struct capref cap);
...
errval_t frame_create_aligned(struct capref dest, size_t bytes, size_t alignment,
                              size_t *retbytes);
errval_t frame_alloc(struct capref *dest, size_t bytes, size_t *retbytes);
errval_t frame_alloc_aligned(struct capref *dest, size_t bytes, size_t alignment,
                             size_t *retbytes);
errval_t frame_free(struct capref cap);
 ...
 */
static inline errval_t cap_mint(struct capref dest, struct capref src, uint64_t param1,
                                uint64_t param2)
{
    ...
    return invoke_cnode_mint(cap_root, dcs_addr, dcn_addr, dest.slot, scp_root, scp_addr,
                             dcn_level, scp_level, param1, param2);
}
...
static inline errval_t vnode_modify_flags(struct capref pgtl, size_t entry,
                                          size_t num_pages, uint64_t attr)
{
    return invoke_vnode_modify_flags(pgtl, entry, num_pages, attr);
}

static inline errval_t vnode_copy_remap(struct capref dest, struct capref src,
                                        capaddr_t slot, uint64_t attr, uint64_t off,
                                        uint64_t pte_count, struct capref mapping)
{
    enum cnode_type slevel = get_cap_level(src);
    capaddr_t saddr = get_cap_addr(src);
    ...
    return invoke_vnode_copy_remap(dest, slot, saddr, slevel, attr, off, pte_count,
                                   mcn_addr, mapping.slot, mcn_level);
}
...
static inline errval_t cap_copy(struct capref dest, struct capref src)
{
    ...
    err = invoke_cnode_copy(cap_root, dcs_addr, dcn_addr, dest.slot, scp_root, scp_addr,
                            dcn_level, scp_level);
    return err;
}
