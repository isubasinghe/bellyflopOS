 ...
 * Create a hash table.
 */
static void collections_hash_create_core(collections_hash_table **t, int num_buckets,
                                         collections_hash_data_free data_free)
{
    ...
    *t = (collections_hash_table *)malloc(sizeof(collections_hash_table));
    ...
    (*t)->buckets = (collections_listnode **)malloc(sizeof(collections_listnode *)
                                                    * num_buckets);
...
void collections_hash_create(collections_hash_table **t,
                             collections_hash_data_free elem_free)
{
    collections_hash_create_core(t, NUM_BUCKETS, elem_free);
}

void collections_hash_create_with_buckets(collections_hash_table **t, int num_buckets,
                                          collections_hash_data_free elem_free)
{
    collections_hash_create_core(t, num_buckets, elem_free);
}
...
static collections_hash_elem *collections_hash_find_elem(collections_hash_table *t,
                                                         uint64_t key)
{
    uint32_t bucket_num;
    collections_listnode *bucket;
...
void collections_hash_insert_or_overwrite(collections_hash_table *t, uint64_t key,
                                          void *data)
{
    uint32_t bucket_num;
    collections_listnode *bucket;
...
void collections_hash_delete(collections_hash_table *t, uint64_t key)
{
    ...
    if (elem) {
        ...
        printf("Error: cannot find the node with key %" PRIu64 " in "
                                                               "collections_hash_"
                                                               "release\n",
               key);
    }
...
collections_hash_get_next_valid_bucket(collections_hash_table *t)
{
    collections_listnode *bucket;
...
{
    struct collections_hash_visitor_tuple *t
        = (struct collections_hash_visitor_tuple *)arg;
    collections_hash_elem *he = (collections_hash_elem *)list_data;
    return t->func(he->key, he->data, t->arg);
}

int collections_hash_visit(collections_hash_table *t, collections_hash_visitor_func func,
                           void *arg)
{
