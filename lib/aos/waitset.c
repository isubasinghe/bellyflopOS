...
static bool waitset_can_receive(struct waitset_chanstate *chan, struct thread *thread)
{
    bool res = false;
    ...
    if (chan->wait_for)  // if a thread is waiting for this specific event
        ...
        res = (chan->token & 1
               && !thread->token)  // incoming token is a request
...
/// our thread
static struct waitset_chanstate *
get_pending_event_disabled(struct waitset *ws, struct waitset_chanstate *waitfor,
                           struct waitset_chanstate *waitfor2)
{
    ...
    if (waitfor) {  // channel that we wait for
        ...
        if (waitfor2
            && (waitfor2->state == CHAN_PENDING || waitfor2->state == CHAN_WAITING)
...
void poll_channels_disabled(dispatcher_handle_t handle)
{
    ...
    do {
        ...
        case CHANTYPE_UMP_IN: {
            struct ump_chan *ump = (struct ump_chan *)chan->data;
            if (chan->waitset != NULL && ump_chan_can_recv(ump)) {
                chan_ready = true;
            }
        } break;
        default:
         ...
         *    This means prev_head == dp->polled_channels and chan is in the queue still
         * so we're fine 2: The dequeued channel was the head but there are more nodes. This
         * means prev_head == dequeued channel && chan != prev_head, so we're fine 3: The
         * dequeued channel was the only node (and the queue is now empty) This means prev_head
         * == dequeued channel && chan == dequeued channel, so we'll exit the loop
         */
...
errval_t get_next_event_disabled(struct waitset *ws, struct waitset_chanstate **retchannel,
            ...
            if (!ws->waiting_threads) {  // no other thread interested in
                ...
                for (t = ws->waiting_threads; t;) {
                    if (waitset_can_receive(chan, t)) {  // match found, wake it
                        ...
                        t = thread_unblock_one_disabled(handle, &ws->waiting_threads,
                                                        chan);
                        assert_disabled(t == NULL);  // shouldn't see a remote thread
                        break;
                    }
...
errval_t get_next_event(struct waitset *ws, struct event_closure *retclosure)
{
    ...
    errval_t err = get_next_event_disabled(ws, &channel, retclosure, NULL, NULL, handle,
                                           false);
    disp_enable(handle);
    return err;
}
...
errval_t event_dispatch_debug(struct waitset *ws)
{
    ...
    struct waitset_chanstate *channel;
    dispatcher_handle_t handle = disp_disable();
    errval_t err = get_next_event_disabled(ws, &channel, &closure, NULL, NULL, handle,
                                           true);
    disp_enable(handle);
    if (err_is_fail(err)) {
        return err;
                          ...
                          errval_t *error_var)
{
    ...
    for (;;) {
        ...
        errval_t err = get_next_event_disabled(ws, &channel, &closure, waitfor,
                                               trigger ? trigger : waitfor->trigger,
                                               handle, false);
        disp_enable(handle);
        if (err_is_fail(err)) {
            assert(0);
...
errval_t event_dispatch_non_block(struct waitset *ws)
{
    ...
    err = get_next_event_disabled(ws, &channel, &closure, NULL, NULL, handle, false);
    if (err_is_fail(err))
        return err;
    disp_enable(handle);
    assert(closure.handler != NULL);
 ...
 */
errval_t waitset_chan_register_disabled(struct waitset *ws, struct waitset_chanstate *chan,
                                        struct event_closure closure)
{
    if (chan->waitset != NULL) {
 ...
 * \param closure Event handler
 */
errval_t waitset_chan_register_polled(struct waitset *ws, struct waitset_chanstate *chan,
                                      struct event_closure closure)
{
    dispatcher_handle_t handle = disp_disable();
 ...
 * \param closure Event handler
 */
errval_t waitset_chan_trigger_closure(struct waitset *ws, struct waitset_chanstate *chan,
                                      struct event_closure closure)
{
...
static int dispatch_loop(void *arg)
{
    struct waitset *ws = (struct waitset *)arg;
    while (true) {
        errval_t err = event_dispatch(ws);
        if (err_is_fail(err)) {
            DEBUG_ERR(err, "Dispatching event failed.\n");
            return 1;
        }
    }
}

// Receives messages from coreid.
void run_dispatcher_threads(size_t n, struct waitset *ws)
{
    for (size_t i = 0; i < n; ++i) {
        thread_create(dispatch_loop, ws);
    }
}