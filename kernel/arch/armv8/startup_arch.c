...
static uint32_t elf_to_l3_flags(uint32_t eflags)
{
    ...
    case PF_W | PF_R:
        return (VMSAv8_64_L3_USR_RW | VMSAv8_64_L3_CACHEABLE | VMSAv8_64_L3_BUFFERABLE);
    case PF_R:
        return (VMSAv8_64_L3_USR_RO | VMSAv8_64_L3_CACHEABLE | VMSAv8_64_L3_BUFFERABLE);
    default:
                                   ...
                                   uint32_t flags, void **ret)
{
    ...
    if (lv > sv && (pa != 0)) {
        spawn_init_map(s2i->l3_table, s2i->l3_base, sv, pa, lv - sv,
                       elf_to_l3_flags(flags));
...
static void load_init_image(struct startup_l3_info *l3i, const char *name,
                            genvaddr_t *init_ep, genvaddr_t *got_base)
{
    lvaddr_t elf_base;
    size_t elf_bytes;
    errval_t err;
    ...
    *init_ep = *got_base = 0;
    ...
    struct multiboot_info *multiboot = (struct multiboot_info *)local_phys_to_mem(
    ...
    err = elf_load(EM_AARCH64, startup_alloc_init, l3i, elf_base, elf_bytes, init_ep);
    if (err_is_fail(err)) {
    ...
    struct Elf64_Shdr *got_shdr = elf64_find_section_header_name((lvaddr_t)elf_base,
                                                                 elf_bytes, ".got");
    if (got_shdr) {
...
void create_module_caps(struct spawn_state *st)
{
    ...
    err = caps_create_new(ObjType_Frame, mmstrings_phys, BASE_PAGE_SIZE, BASE_PAGE_SIZE,
                          my_core_id,
                          caps_locate_slot(CNODE(st->modulecn), st->modulecn_slot++));
    ...
    acpi_new = (struct multiboot_tag_new_acpi *)multiboot2_find_tag(
        multiboot->tags, multiboot->total_size - 8, MULTIBOOT_TAG_TYPE_ACPI_NEW);
    if (acpi_new) {
        acpi_base = mem_to_local_phys((lvaddr_t)&acpi_new->rsdp[0]);
    } else {
        struct multiboot_tag_old_acpi *acpi_old;
        acpi_old = (struct multiboot_tag_old_acpi *)multiboot2_find_tag(
            multiboot->tags, multiboot->total_size - 8, MULTIBOOT_TAG_TYPE_ACPI_OLD);
        if (acpi_old) {
    ...
    struct multiboot_tag_module_64 *module
        = (struct multiboot_tag_module_64 *)multiboot2_find_tag(
            multiboot->tags, multiboot->total_size - 8, MULTIBOOT_TAG_TYPE_MODULE_64);
    while (module) {
        ...
        err = caps_create_new(ObjType_DevFrame, base_addr, remain, remain, my_core_id,
                              caps_locate_slot(CNODE(st->modulecn), st->modulecn_slot++));
        assert(err_is_ok(err));
...
static void create_phys_caps_region(lpaddr_t reserved_start, lpaddr_t reserved_end,
                                    lpaddr_t region_base, size_t region_size,
                                    enum region_type region_type)
{
    ...
    if (reserved_start <= region_base + region_size && region_base <= reserved_end) {
        ...
        if (region_base < reserved_start) {
            err = create_caps_to_cnode(region_base, reserved_start - region_base,
                                       region_type, &spawn_state, bootinfo);
        }
        assert(err_is_ok(err));
        if (region_base + region_size > reserved_end) {
            err = create_caps_to_cnode(reserved_end,
                                       region_base + region_size - reserved_end,
                                       region_type, &spawn_state, bootinfo);
        }
    } else {
        err = create_caps_to_cnode(region_base, region_size, region_type, &spawn_state,
                                   bootinfo);
    }
    assert(err_is_ok(err));
}
...
static void create_phys_caps(lpaddr_t reserved_start, lpaddr_t reserved_end)
{
    ...
    for (size_t i = 0;
         i < (mmap->size - sizeof(struct multiboot_tag_efi_mmap)) / mmap->descr_size;
         i++) {
        efi_memory_descriptor *desc = (efi_memory_descriptor *)(mmap->efi_mmap
                                                                + mmap->descr_size * i);
        ...
        if (last_end_addr < desc->PhysicalStart) {
            ...
            create_phys_caps_region(reserved_start, reserved_end, last_end_addr,
                                    desc->PhysicalStart - last_end_addr,
                                    RegionType_PhyAddr);
        }
        last_end_addr = desc->PhysicalStart + desc->NumberOfPages * BASE_PAGE_SIZE;

        create_phys_caps_region(reserved_start, reserved_end, desc->PhysicalStart,
                                desc->NumberOfPages * BASE_PAGE_SIZE, region_type);
    }

    size_t size = (1UL << 48) - last_end_addr;


    create_phys_caps_region(reserved_start, reserved_end, last_end_addr, size,
                            RegionType_PhyAddr);
}
...
static void init_page_tables(void)
{
    ...
    const size_t l0_size = VMSAv8_64_PTABLE_NUM_ENTRIES * INIT_L0_SIZE
                           * sizeof(union armv8_ttable_entry);
    init_l0 = (void *)local_phys_to_mem(
        alloc_phys_aligned(l0_size, VMSAv8_64_PTABLE_SIZE));
    memset(init_l0, 0, l0_size);

    const size_t l1_size = l0_size * INIT_L1_SIZE;
    init_l1 = (void *)local_phys_to_mem(
        alloc_phys_aligned(l1_size, VMSAv8_64_PTABLE_SIZE));
    memset(init_l1, 0, l1_size);

    const size_t l2_size = l1_size * INIT_L2_SIZE;
    init_l2 = (void *)local_phys_to_mem(
        alloc_phys_aligned(l2_size, VMSAv8_64_PTABLE_SIZE));
    memset(init_l2, 0, l2_size);

    const size_t l3_size = l2_size * INIT_L3_SIZE;
    init_l3 = (void *)local_phys_to_mem(
        alloc_phys_aligned(l3_size, VMSAv8_64_PTABLE_SIZE));
    ...
    caps_create_new(ObjType_VNode_AARCH64_l0, mem_to_local_phys((lvaddr_t)init_l0),
                    vnode_objsize(ObjType_VNode_AARCH64_l0), 0, my_core_id,
    ...
    for (size_t i = 0; i < INIT_L1_SIZE; i++) {
        ...
        caps_create_new(ObjType_VNode_AARCH64_l1,
    ...
    for (size_t i = 0; i < INIT_L2_SIZE; i++) {
        ...
        assert(objsize_vnode == BASE_PAGE_SIZE);
        caps_create_new(ObjType_VNode_AARCH64_l2,
    ...
    for (size_t i = 0; i < INIT_L3_SIZE; i++) {
        ...
        assert(objsize_vnode == BASE_PAGE_SIZE);
        caps_create_new(ObjType_VNode_AARCH64_l3,
     ...
     */
    for (lvaddr_t vaddr = ARMV8_INIT_VBASE; vaddr < ARMV8_INIT_SPACE_LIMIT;
...
static struct dcb *spawn_init_common(const char *name, int argc, const char *argv[],
                                     ...
                                     alloc_phys_aligned_func alloc_phys_aligned)
{
    ...
    lvaddr_t paramaddr;
    struct dcb *init_dcb = spawn_module(&spawn_state, name, argc, argv, bootinfo_phys,
                                        INIT_ARGS_VBASE, alloc_phys, alloc_phys_aligned,
                                        &paramaddr);
    ...
    spawn_init_map(init_l3, ARMV8_INIT_VBASE, INIT_ARGS_VBASE, spawn_state.args_page,
                   ARGS_SIZE, INIT_PERM_RW);

    /* Map dispatcher */
    spawn_init_map(init_l3, ARMV8_INIT_VBASE, INIT_DISPATCHER_VBASE,
                   mem_to_local_phys(init_dcb->disp), DISPATCHER_FRAME_SIZE, INIT_PERM_RW);
...
struct dcb *spawn_bsp_init(const char *name)
{
    ...
    struct dispatcher_shared_aarch64 *disp_aarch64 = get_dispatcher_shared_aarch64(
        init_dcb->disp);
...
struct dcb *spawn_app_init(struct armv8_core_data *core_data, const char *name)
{
    ...
    err = caps_create_new(ObjType_DevFrame, core_data->urpc_frame.base,
                          core_data->urpc_frame.length, core_data->urpc_frame.length,
    ...
    spawn_init_map(init_l3, ARMV8_INIT_VBASE, MON_URPC_VBASE, urpc_ptr, MON_URPC_SIZE,
                   INIT_PERM_RW);
    ...
    got_shdr = elf64_find_section_header_name(
        local_phys_to_mem(core_data->monitor_binary.base),
