...
/**
 * \brief Set the mem rpc channel on the domain state
 */
void set_mem_server_rpc(struct aos_rpc *mem_server)
{
    dispatcher_handle_t handle = curdispatcher();
    struct dispatcher_generic *disp = get_dispatcher_generic(handle);
    disp->core_state.c.mem_server_rpc = mem_server;
}

/*
 * \brief Set core client rpc channel on the domain state
 */
void set_core_client_rpc(coreid_t coreid, struct aos_rpc *core_rpc)
{
    dispatcher_handle_t handle = curdispatcher();
    struct dispatcher_generic *disp = get_dispatcher_generic(handle);
    disp->core_state.c.core_client_rpcs[coreid] = core_rpc;
}

/*
 * \brief Set core server rpc channel on the domain state
 */
void set_core_server_rpc(coreid_t coreid, struct aos_rpc *core_rpc)
{
    dispatcher_handle_t handle = curdispatcher();
    struct dispatcher_generic *disp = get_dispatcher_generic(handle);
    disp->core_state.c.core_server_rpcs[coreid] = core_rpc;
}

struct aos_rpc *get_core_client_rpc(coreid_t coreid)
{
    dispatcher_handle_t handle = curdispatcher();
    struct dispatcher_generic *disp = get_dispatcher_generic(handle);
    return disp->core_state.c.core_client_rpcs[coreid];
}

struct aos_rpc *get_core_server_rpc(coreid_t coreid)
{
    dispatcher_handle_t handle = curdispatcher();
    struct dispatcher_generic *disp = get_dispatcher_generic(handle);
    return disp->core_state.c.core_server_rpcs[coreid];
}

void set_nameserver_chan(struct nameservice_chan *chan)
{
    dispatcher_handle_t handle = curdispatcher();
    struct dispatcher_generic *disp = get_dispatcher_generic(handle);
    disp->core_state.c.nameserver_chan = chan;
}

struct nameservice_chan *get_nameserver_chan(void)
{
    dispatcher_handle_t handle = curdispatcher();
    struct dispatcher_generic *disp = get_dispatcher_generic(handle);
    return disp->core_state.c.nameserver_chan;
}

void set_is_nameserver_started(bool is_started)
{
    dispatcher_handle_t handle = curdispatcher();
    struct dispatcher_generic *disp = get_dispatcher_generic(handle);
    disp->core_state.c.is_nameserver_started = is_started;
}

bool get_is_nameserver_started(void)
{
    dispatcher_handle_t handle = curdispatcher();
    struct dispatcher_generic *disp = get_dispatcher_generic(handle);
    return disp->core_state.c.is_nameserver_started;
}
...
/**
 * \brief Returns the RPC channel to mem server
 */
struct aos_rpc *get_mem_server_rpc(void)
{
    dispatcher_handle_t handle = curdispatcher();
    struct dispatcher_generic *disp = get_dispatcher_generic(handle);
    return disp->core_state.c.mem_server_rpc;
}
