...
{
    errval_t err = aos_rpc_get_ram_cap(aos_rpc_get_memory_channel(), size, alignment, ret,
                                       NULL);
    if (err_is_fail(err)) {
...
errval_t ram_free(struct capref cap)
{
    ...
    errval_t err = ram_alloc_state->ram_free_func(cap);
    return err;
}
...
errval_t ram_free_set(ram_free_func_t ram_free_func)
{
    // Nono there is no ram_alloc_remote yet. So dont use it remotely!
    struct ram_alloc_state *ram_alloc_state = get_ram_alloc_state();
    ram_alloc_state->ram_free_func = ram_free_func;
    return SYS_ERR_OK;
}
