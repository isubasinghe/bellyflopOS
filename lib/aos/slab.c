...
static errval_t slab_refill_pages(struct slab_allocator *slabs, size_t bytes)
{
    ...
    struct capref cap;
    size_t retbytes;
    errval_t err = frame_alloc(&cap, bytes, &retbytes);
    if (err_is_fail(err)) {
        return err;
    }
    void *buf;
    err = paging_map_frame_attr(get_current_paging_state(), &buf, retbytes, cap,
    ...
    // This is the implementation from bugfixed handout. When merging I'd rather stuck
    // to the original we had. Feel free to test this and incorporate.
    // errval_t err;
    // struct capref cap;
                                  ...
                                  size_t minbytes)
{
    ...
    size_t retbytes;
    errval_t err = frame_create(frame_slot, bytes, &retbytes);
    if (err_is_fail(err)) {
        return err_push(err, LIB_ERR_FRAME_CREATE);
    }

    void *buf = NULL;
    err = paging_map_frame_complete(get_current_paging_state(), &buf, frame_slot);
    if (err_is_fail(err)) {
        return err;
    }
