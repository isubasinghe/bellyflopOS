...
enum ws_chanstate {
    ...
    CHAN_POLLED,  ///< Idle and polled. Channel implementation must be called to check for
                  ///< pending events
...
errval_t wait_for_channel(struct waitset *ws, struct waitset_chanstate *channel,
                          errval_t *error_var);
