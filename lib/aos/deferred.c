 ...
 */
errval_t deferred_event_register(struct deferred_event *event, struct waitset *ws,
                                 delayus_t delay, struct event_closure closure)
{
    errval_t err;
...
errval_t barrelfish_usleep(delayus_t delay)
{
    ...
    waitset_init(&ws);
    err = deferred_event_register(&dev, &ws, delay, MKCLOSURE(usleep_callback, &wakeup));
    if (err_is_fail(err)) {
        return err;
    }
...
errval_t wait_for_or_timeout(delayus_t delay, deferred_event_checker_t check, void *arg)
{
    ...
    //     return err;
    // }
    uint64_t begin = systime_to_us(systime_now());
    uint64_t now = begin;
    while (now < begin  + delay && !check(arg)) {
        now = systime_to_us(systime_now());
    }
