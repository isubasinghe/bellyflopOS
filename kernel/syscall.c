...
#    include <arch/armv8/psci.h>
struct sysret sys_cpu_suspend(void)
{
    printf("Suspending current core.");
    errval_t err = psci_cpu_off();
    return SYSRET(err);
}

void sys_cpu_resume(coreid_t core)
{
    // How to send an interrupt to another core?
}
...
struct sysret sys_dispatcher_setup(struct capability *to, capaddr_t cptr, uint8_t level,
                                   capaddr_t vptr, capaddr_t dptr, bool run,
                                   capaddr_t odptr)
{
    ...
    /* 1. set cspace root */
    struct cte *root;
    err = caps_lookup_slot(&dcb_current->cspace.cap, cptr, level, &root, CAPRIGHTS_READ);
    if (err_is_fail(err)) {
        debug(SUBSYS_CAPS,
              "caps_lookup_cap for croot=%" PRIxCADDR ", level=%d: %" PRIuERRV "\n", cptr,
              level, err);
    ...
    err = caps_lookup_cap(&dcb_current->cspace.cap, odptr, CNODE_TYPE_COUNT, &odisp,
                          CAPRIGHTS_READ);
    if (err_is_fail(err)) {
    ...
    struct dispatcher_shared_generic *disp = get_dispatcher_shared_generic(dcb->disp);
...
struct sysret sys_dispatcher_properties(struct capability *to, enum task_type type,
                                        unsigned long deadline, unsigned long wcet,
                                        unsigned long period, unsigned long release,
                                        unsigned short weight)
{
    ...
    trace_event(TRACE_SUBSYS_KERNEL, TRACE_EVENT_KERNEL_SCHED_REMOVE, 152);
...
struct sysret sys_dispatcher_stop(struct capability *disp_cap)
{
    assert(disp_cap->type == ObjType_Dispatcher);
    scheduler_remove(disp_cap->u.dispatcher.dcb);
    return SYSRET(SYS_ERR_OK);
}
 ...
 */
struct sysret sys_retype(struct capability *root, capaddr_t source_croot,
                         capaddr_t source_cptr, gensize_t offset, enum objtype type,
                         gensize_t objsize, size_t count, capaddr_t dest_cspace_cptr,
                         capaddr_t dest_cnode_cptr, uint8_t dest_cnode_level,
                         cslot_t dest_slot, bool from_monitor)
{
    errval_t err;
    ...
    err = caps_lookup_slot(source_root, source_cptr, 2, &source_cte, CAPRIGHTS_READ);
    if (err_is_fail(err)) {
    ...
    return SYSRET(caps_retype(type, objsize, count, dest_cnode_cap, dest_slot, source_cte,
                              offset, from_monitor));
}

struct sysret sys_create(struct capability *root, enum objtype type, size_t objsize,
                         capaddr_t dest_cnode_cptr, uint8_t dest_level, cslot_t dest_slot)
{
    errval_t err;
    uint8_t size = 0;
    genpaddr_t base = 0;
    ...
    /* Destination CNode */
    struct capability *dest_cnode_cap;
    err = caps_lookup_cap(root, dest_cnode_cptr, dest_level, &dest_cnode_cap,
                          CAPRIGHTS_READ_WRITE);
    if (err_is_fail(err)) {
        return SYSRET(err_push(err, SYS_ERR_DEST_CNODE_LOOKUP));
    }
 ...
 */
struct sysret sys_copy_or_mint(struct capability *root, capaddr_t dest_cspace_cptr,
                               capaddr_t destcn_cptr, cslot_t dest_slot,
                               capaddr_t source_croot_ptr, capaddr_t source_cptr,
                               ...
                               uintptr_t param1, uintptr_t param2, bool mint)
{
    ...
    err = caps_lookup_cap(root, source_croot_ptr, 2, &src_croot, CAPRIGHTS_READ);
    if (err_is_fail(err)) {
        return SYSRET(err_push(err, SYS_ERR_SOURCE_ROOTCN_LOOKUP));
    }
    ...
    struct cte *src_cap;
    err = caps_lookup_slot(src_croot, source_cptr, source_level, &src_cap, CAPRIGHTS_READ);
    if (err_is_fail(err)) {
        return SYSRET(err_push(err, SYS_ERR_SOURCE_CAP_LOOKUP));
    }
    ...
    err = caps_lookup_slot(dest_cspace_root, destcn_cptr, destcn_level, &dest_cnode_cap,
                           CAPRIGHTS_READ_WRITE);
    if (err_is_fail(err)) {
        return SYSRET(err_push(err, SYS_ERR_DEST_CNODE_LOOKUP));
    }
    ...
    if (dest_cnode_cap->cap.type == ObjType_L1CNode
        ...
        return SYSRET(
            caps_copy_to_cnode(dest_cnode_cap, dest_slot, src_cap, mint, param1, param2));
    } else {
        return SYSRET(SYS_ERR_DEST_TYPE_INVALID);
    }
}

struct sysret sys_map(struct capability *ptable, cslot_t slot, capaddr_t source_root_cptr,
                      ...
                      capaddr_t mapping_cnptr, uint8_t mapping_cn_level,
                      cslot_t mapping_slot)
{
    ...
    err = caps_lookup_cap(root, source_root_cptr, source_level, &src_root, CAPRIGHTS_READ);
    if (err_is_fail(err)) {
        return SYSRET(err_push(err, SYS_ERR_SOURCE_ROOTCN_LOOKUP));
    }
    ...
    /* Lookup source cap in source cspace */
    struct cte *src_cte;
    err = caps_lookup_slot(src_root, source_cptr, source_level, &src_cte, CAPRIGHTS_READ);
    if (err_is_fail(err)) {
        return SYSRET(err_push(err, SYS_ERR_SOURCE_CAP_LOOKUP));
    }
    ...
    err = caps_lookup_cap(root, mapping_crootptr, 2, &mapping_croot, CAPRIGHTS_READ_WRITE);
    if (err_is_fail(err)) {
        return SYSRET(err_push(err, SYS_ERR_DEST_ROOTCN_LOOKUP));
    }
    ...
    return SYSRET(caps_copy_to_vnode(cte_for_cap(ptable), slot, src_cte, flags, offset,
                                     pte_count, mapping_cte));
}

struct sysret sys_copy_remap(struct capability *ptable, cslot_t slot,
                             capaddr_t source_cptr, int source_level, uintptr_t flags,
                             ...
                             cslot_t mapping_slot)
{
    ...
    struct capability *root = &dcb_current->cspace.cap;
    struct cte *src_cte;
    err = caps_lookup_slot(root, source_cptr, source_level, &src_cte, CAPRIGHTS_READ);
    if (err_is_fail(err)) {
        return SYSRET(err_push(err, SYS_ERR_SOURCE_CAP_LOOKUP));
    }
    ...
    struct cte *mapping_cnode_cte;
    err = caps_lookup_slot(root, mapping_cnptr, mapping_cn_level, &mapping_cnode_cte,
                           CAPRIGHTS_READ_WRITE);
    if (err_is_fail(err)) {
        return SYSRET(err_push(err, SYS_ERR_DEST_CNODE_LOOKUP));
    }
    ...
    // XXX: this does not check if we do have CAPRIGHTS_READ_WRITE on
    // the destination cap (the page table we're inserting into)
    return SYSRET(paging_copy_remap(cte_for_cap(ptable), slot, src_cte, flags, offset,
                                    pte_count, mapping_cte));
}
                                 ...
                                 capaddr_t retcn_cptr, cslot_t retslot)
{
    ...
    struct cte *taskcn = caps_locate_slot(get_address(&newroot->cap), ROOTCN_SLOT_TASKCN);
 ...
 */
struct sysret sys_identify_cap(struct capability *root, capaddr_t cptr, uint8_t level,
...
struct sysret sys_yield(capaddr_t target)
{
    ...
    debug(SUBSYS_DISPATCH, "%.*s yields%s\n", DISP_NAME_LEN, disp->name,
          !disp->haswork && disp->lmp_delivered == disp->lmp_seen ? " and is removed "
                                                                    "from the runq"
                                                                  : "");
    ...
    if (target != CPTR_NULL) {
        ...
        /* directed yield */
        err = caps_lookup_cap(&dcb_current->cspace.cap, target, 2, &yield_to,
                              CAPRIGHTS_READ);
        if (err_is_fail(err)) {
            return SYSRET(err);
    ...
    if (!disp->haswork && disp->lmp_delivered == disp->lmp_seen
        ...
        trace_event(TRACE_SUBSYS_KERNEL, TRACE_EVENT_KERNEL_SCHED_REMOVE, 151);
