...
static int fs_libc_open(char *path, int flags)
{
    ...
    if (flags & O_CREAT) {
        ...
        if (flags & O_EXCL) {
            ...
            err = fatfs_open(mount, path, flags, &vh);
        ...
        err = fatfs_create(mount, path, flags, &vh);
...
static int fs_libc_read(int fd, void *buf, size_t len)
{
    ...
    case FDTAB_TYPE_FILE: {
        ...
        if (err_is_fail(err)) {
            DEBUG_ERR(err, "fatfs read failed\n");
            return -1;
        }
    } break;
    default:
...
// static errval_t fs_opendir(const char *path, fs_dirhandle_t *h){ return
// ramfs_opendir(mount, path, h); } static errval_t fs_readdir(fs_dirhandle_t h, char
// **name) { return ramfs_dir_read_next(mount, h, name, NULL); } static errval_t
// fs_closedir(fs_dirhandle_t h) { return ramfs_closedir(mount, h); } static errval_t
// fs_fstat(fs_dirhandle_t h, struct fs_fileinfo *b) { return ramfs_stat(mount, h, b); }

static errval_t fs_mkdir(const char *path)
{
    return fatfs_mkdir(mount, path);
}
static errval_t fs_rmdir(const char *path)
{
    return fatfs_rmdir(mount, path);
}
static errval_t fs_rm(const char *path)
{
    return fatfs_remove(mount, path);
}
static errval_t fs_opendir(const char *path, fs_dirhandle_t *h)
{
    return fatfs_opendir(mount, path, h);
}
static errval_t fs_readdir(fs_dirhandle_t h, char **name)
{
    return fatfs_dir_read_next(mount, h, name, NULL);
}
static errval_t fs_closedir(fs_dirhandle_t h)
{
    return fatfs_closedir(mount, h);
}
static errval_t fs_fstat(fs_dirhandle_t h, struct fs_fileinfo *b)
{
    return fatfs_stat(mount, h, b);
}
