...
int main(int argc, char *argv[])
{
    ...
    err = map_device_register(IMX8X_ENET_BASE, IMX8X_ENET_SIZE, task_cap_argcn0,
                              &st->regs, &st->d_vaddr);
    if (err_is_fail(err)) {
        USER_PANIC_ERR(err, "Failed to map ENET registers\n");
    }
    ...
    err = init_ethernet_handler(st);
    RETURN_IF_ERR(err);

    init_arp_handler();

    init_netsocket_servers();

    tcp_init();

    arp_probe();

    // arp_request(str_to_ip_addr("10.0.2.2"));
    // arp_request(str_to_ip_addr("8.8.8.8"));
    ...
    while (true) {
        ...
        if (err_is_ok(err)) {
            handle_ethernet(&buf, buf.valid_length);
