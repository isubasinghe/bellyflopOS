 ...
 * \return Error code
 */
static inline errval_t invoke_cnode_create(struct capref root, enum objtype type,
                                           size_t objsize, capaddr_t dest_cnode_cptr,
                                           enum cnode_type dest_level, capaddr_t dest_slot)
{
    assert(dest_cnode_cptr != CPTR_NULL);
    return cap_invoke6(root, CNodeCmd_Create, type, objsize, dest_cnode_cptr, dest_level,
                       dest_slot)
        .error;
}
...
invoke_cnode_mint(struct capref root, capaddr_t to_cspace, capaddr_t to, capaddr_t slot,
                  capaddr_t from_cspace, capaddr_t from, enum cnode_type tolevel,
                  enum cnode_type fromlevel, uint64_t param1, uint64_t param2)
{
    return cap_invoke10(root, CNodeCmd_Mint, to_cspace, to, slot, from_cspace, from,
                        tolevel, fromlevel, param1, param2)
        .error;
}
...
{
    return cap_invoke8(root, CNodeCmd_Copy, to_cspace, to, slot, from_cspace, from,
                       tolevel, fromlevel)
        .error;
}
...
static inline errval_t invoke_vnode_modify_flags(struct capref cap, size_t entry,
                                                 size_t num_pages, size_t attr)
{
...
static inline errval_t invoke_vnode_copy_remap(struct capref ptable, capaddr_t slot,
                                               ...
                                               size_t flags, size_t offset,
                                               size_t pte_count, capaddr_t mcn_addr,
                                               cslot_t mapping_slot,
                                               enum cnode_type mcn_level)
{
    return cap_invoke10(ptable, VNodeCmd_CopyRemap, slot, src, srclevel, flags, offset,
                        pte_count, mcn_addr, mapping_slot, mcn_level)
        .error;
}
 ...
 * \return Error code
 */
static inline errval_t invoke_kcb_identify(struct capref kcb, struct frame_identity *ret)
{
    assert(ret != NULL);
 ...
 * \return Error code
 */
static inline errval_t invoke_cap_identify(struct capref cap, struct capability *ret)
{
    struct capref croot = get_croot_capref(cap);
    return cap_invoke4(croot, CNodeCmd_CapIdentify, get_cap_addr(cap), get_cap_level(cap),
                       (uintptr_t)ret)
        .error;
}
 ...
 * \return Error code
 */
static inline errval_t invoke_mapping_modify_flags(struct capref mapping, size_t offset,
                                                   size_t pages, size_t flags,
                                                   genvaddr_t va_hint)
{
    return cap_invoke5(mapping, MappingCmd_Modify, offset, pages, flags, va_hint).error;
}
                                         ...
                                         struct capref dispframe, bool run)
{
    ...
    return cap_invoke7(dispatcher, DispatcherCmd_Setup, root_caddr, root_level,
                       vtree_caddr, disp_caddr, run, dd_caddr)
        .error;
}
...
static inline errval_t
invoke_dispatcher_properties(struct capref dispatcher, enum task_type type,
                             unsigned long deadline, unsigned long wcet,
                             unsigned long period, unsigned long release,
                             unsigned short weight)
{
    return cap_invoke7(dispatcher, DispatcherCmd_Properties, type, deadline, wcet, period,
                       release, weight)
        .error;
}
...
static inline errval_t invoke_irqtable_alloc_dest_cap(struct capref irqcap,
                                                      struct capref dest_cap, int vec_hint)
{
    uint8_t dcn_level = get_cnode_level(dest_cap);
    capaddr_t dcn_addr = get_cnode_addr(dest_cap);
    struct sysret ret = cap_invoke5(irqcap, IRQTableCmd_AllocDestCap, dcn_level, dcn_addr,
                                    dest_cap.slot, vec_hint);
    return ret.error;
}
...
static inline errval_t invoke_irqtable_set(struct capref irqcap, int irq, struct capref ep)
{
...
static inline errval_t invoke_kernel_get_platform(struct platform_info *pi)
{
    return cap_invoke2(cap_kernel, KernelCmd_Get_platform, (uintptr_t)pi).error;
}
