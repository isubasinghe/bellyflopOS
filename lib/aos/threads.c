...
/// Lock `thread_slabs_spinlock` must be acquired prior to calling this function.
...
static errval_t refill_thread_slabs(struct slab_allocator *slabs)
{
    ...
    errval_t err = paging_map_lazy(st, &buf, size, BASE_PAGE_SIZE,
                                        ...
                                        size_t stacksize)
{
    ...
    */
    ...
    // Allocate stack. Note that it's not lazy-mapped. This is because if stack grows
    // in disabled mode the pagefault handler cannot back it.
    struct paging_state *st = get_current_paging_state();
    void *stack;
    struct capref stack_cap;

    err = frame_alloc_aligned(&stack_cap, stacksize, BASE_PAGE_SIZE, NULL);
    if (err_is_fail(err)) {
        free_thread(newthread);
        return NULL;
    }

    err = paging_map_frame_complete(st, &stack, stack_cap);
    if (err_is_fail(err)) {
        free_thread(newthread);
        return NULL;
    }
...
static int main_thread(void *params)
{
    ...
    grading_setup_noninit(&p->argc, (char ***)&p->argv);
    grading_test_early();
    exit(_main(p->argc, p->argv));
    // We are really  not sure where to call this is this to late?
    grading_test_late();
    return EXIT_FAILURE;
}
