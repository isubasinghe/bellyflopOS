...
struct capref task_cap_argcn0 = { .cnode = TASK_CNODE_INIT, .slot = TASKCN_SLOT_ARGCN0 };
struct capref task_cap_argcn1 = { .cnode = TASK_CNODE_INIT, .slot = TASKCN_SLOT_ARGCN1 };
struct capref task_cap_argcn2 = { .cnode = TASK_CNODE_INIT, .slot = TASKCN_SLOT_ARGCN2 };
...
struct capref cap_memep = { .cnode = TASK_CNODE_INIT, .slot = TASKCN_SLOT_MEMEP };
...
/**
 * \brief Returns physical address of the capability.
 *
...
{
    struct capability c;
    errval_t err = invoke_cap_identify(cap, &c);
    assert(!err_is_fail(err) && "Passed wrong capability to get_frame_paddr()");
    return get_address(&c);
}

/**
 * \brief Returns physical size of the capability.
 *
 * \param cap           Capability
 */
...
/*
* \brief Frees a frame create with frame_alloc or frame_alloc_aligned
*/
