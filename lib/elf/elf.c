                      ...
                      size_t *ret_tlstotallen)
{
    ...
    case ELFCLASS32:
        return elf32_load(em_machine, allocate_func, state, base, size, retentry,
                          ret_tlsbase, ret_tlsinitlen, ret_tlstotallen);

    case ELFCLASS64:
        return elf64_load(em_machine, allocate_func, state, base, size, retentry,
                          ret_tlsbase, ret_tlsinitlen, ret_tlstotallen);
    }
...
errval_t elf_load(uint16_t em_machine, elf_allocator_fn allocate_func, void *state,
                  lvaddr_t base, size_t size, genvaddr_t *retentry)
{
    return elf_load_tls(em_machine, allocate_func, state, base, size, retentry, NULL,
                        NULL, NULL);
}
...
size_t elf_number_of_segments(size_t segment_type, lvaddr_t base, size_t size)
{
    ...
    struct Elf64_Phdr *phead = (struct Elf64_Phdr *)((uintptr_t)ehead
                                                     + (uintptr_t)ehead->e_phoff);

    size_t retval = 0;
    for (int i = 0; i < ehead->e_phnum; i++) {
...
{
    struct Elf32_Phdr *phead = (struct Elf32_Phdr *)((uintptr_t)ehead
                                                     + (uintptr_t)ehead->e_phoff);
...
static errval_t elf32_get_eh_info(lvaddr_t elfbase, size_t elfsize, lvaddr_t *eh_frame,
...
static errval_t elf64_get_eh_info(lvaddr_t elfbase, size_t elfsize, lvaddr_t *eh_frame,
                                  size_t *eh_frame_size, lvaddr_t *eh_frame_hdr,
                                  size_t *eh_frame_hdr_size)
{
