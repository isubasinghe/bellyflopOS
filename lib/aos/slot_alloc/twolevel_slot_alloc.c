...
errval_t two_level_alloc(struct slot_allocator *ca, struct capref *ret)
{
    ...
    if (ca->space == 0) {
        ...
        // TODO: Not sure if this is a bug. The problem here is that slot_alloc.c in mm
        // also does a root slot alloc but does not refill it if there was only one slot
        // before. Maybe one needs to add a check there like its done here to prevent
        // this.
        if (rootcn_free <= 2) {
        ...
        if (!buf) { /* Grow slab */
            ...
            err = slab_refill_no_pagefault(&mca->slab, frame,
                                           SLAB_STATIC_SIZE(1, mca->slab.blocksize));
            if (err_is_fail(err)) {
        ...
        // Allocator
        err = single_slot_alloc_init_raw(&mca->reserve->a, cap, cnode, mca->a.nslots, buf,
                                         bufsize);
        if (err_is_fail(err)) {
            thread_mutex_unlock(&ca->mutex);
...
errval_t two_level_slot_alloc_init_raw(struct multi_slot_allocator *ret,
                                       ...
                                       struct cnoderef reserve_cnode, void *head_buf,
                                       void *reserve_buf, size_t bufsize)
{
    ...
    /* Head */
    err = single_slot_alloc_init_raw(&ret->head->a, initial_cap, initial_cnode,
                                     L2_CNODE_SLOTS, head_buf, bufsize);
    if (err_is_fail(err)) {
        return err_push(err, LIB_ERR_SINGLE_SLOT_ALLOC_INIT);
    }

    /* Reserve */
    err = single_slot_alloc_init_raw(&ret->reserve->a, reserve_cap, reserve_cnode,
                                     L2_CNODE_SLOTS, reserve_buf, bufsize);
    if (err_is_fail(err)) {
        return err_push(err, LIB_ERR_SINGLE_SLOT_ALLOC_INIT);
    }
...
errval_t two_level_slot_alloc_init(struct multi_slot_allocator *ret)
{
    ...
    err = two_level_slot_alloc_init_raw(ret, initial_cap, initial_cnode, reserve_cap,
                                        reserve_cnode, head_buf, reserve_buf, bufsize);
    return SYS_ERR_OK;
}
