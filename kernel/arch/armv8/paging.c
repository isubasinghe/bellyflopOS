...
static void paging_write_l0_entry(union armv8_ttable_entry *l0_table, lvaddr_t va,
                                  union armv8_ttable_entry l0)
{
    assert(l0_table);
...
static void paging_write_l1_entry(union armv8_ttable_entry *l1_table, lvaddr_t va,
...
static void paging_write_l2_entry(union armv8_ttable_entry *l2_table, lvaddr_t va,
...
static void paging_write_l3_entry(union armv8_ttable_entry *l3_table, lvaddr_t va,
                                  union armv8_ttable_entry l3)
{
    assert(l3_table);
...
void paging_map_memory(union armv8_ttable_entry *ttbase, lpaddr_t paddr, size_t bytes)
{
    ...
    while (paddr < pend) {
        paging_map_kernel_l1_block(ttbase, (paddr + MEMORY_OFFSET) - KERNEL_OFFSET, paddr);
...
void paging_map_block_l1(union armv8_ttable_entry *table_base, lvaddr_t va, lpaddr_t pa,
                         uintptr_t flags)
{
    assert(aligned((uintptr_t)table_base, VMSAv8_64_PTABLE_SIZE));
    assert(aligned(pa, VMSAv8_64_PTABLE_SIZE));
...
void paging_map_block_l2(union armv8_ttable_entry *table_base, lvaddr_t va, lpaddr_t pa,
                         uintptr_t flags)
{
    assert(aligned((uintptr_t)table_base, VMSAv8_64_PTABLE_SIZE));
    assert(aligned(pa, VMSAv8_64_PTABLE_SIZE));
...
void paging_map_page_l3(union armv8_ttable_entry *table_base, lvaddr_t va, lpaddr_t pa,
                        uintptr_t flags)
{
    assert(aligned((uintptr_t)table_base, VMSAv8_64_PTABLE_SIZE));
    assert(aligned(pa, VMSAv8_64_PTABLE_SIZE));
...
void paging_set_l3_entry(union armv8_ttable_entry *l3_entry, lpaddr_t pa, uintptr_t flags)
{
    ...
    //    e.page.base << BASE_PAGE_BITS, e.raw);
...
static void paging_set_flags(union armv8_ttable_entry *entry, uintptr_t kpi_paging_flags)
{
    entry->page.sh = 3;
    entry->page.ap = 0;
...
static errval_t caps_map_l0(struct capability *dest, cslot_t slot, struct capability *src,
...
static errval_t caps_map_l1(struct capability *dest, cslot_t slot, struct capability *src,
                            uintptr_t kpi_paging_flags, uintptr_t offset,
                            uintptr_t pte_count, struct cte *mapping_cte)
{
    if (slot >= VMSAv8_64_PTABLE_NUM_ENTRIES) {
                            ...
                            uintptr_t pte_count, struct cte *mapping_cte)
{
    ...
    case ObjType_DevFrame:
        ...
        for (int i = 0; i < pte_count; i++) {
            if (entry[i].d.valid) {
                // cleanup mapping info
                debug(SUBSYS_PAGING, "ARMv8 L2 superpage @ 0x%lx: slot %d in use\n",
                      dest_lpaddr, slot + i);
                return SYS_ERR_VNODE_SLOT_INUSE;
            }
        }
        ...
        for (int i = 0; i < pte_count; i++) {
            ...
            entry->block_l2.base = ((src_lpaddr + i * LARGE_PAGE_SIZE) >> LARGE_PAGE_BITS);


            debug(SUBSYS_PAGING,
                  "L2 Superpage mapping %08" PRIxLVADDR "[%" PRIuCSLOT "] @%p = "
                                                                       "%08" PRIx64 "\n",
...
static errval_t caps_map_l3(struct capability *dest, cslot_t slot, struct capability *src,
                            uintptr_t kpi_paging_flags, uintptr_t offset,
                            uintptr_t pte_count, struct cte *mapping_cte)
{
    ...
    for (int i = 0; i < pte_count; i++) {
        ...
        debug(SUBSYS_PAGING,
              "L3 mapping %08" PRIxLVADDR "[%" PRIuCSLOT "] @%p = %08" PRIx64 "\n",
              dest_lvaddr, slot, entry, entry->raw);
...
typedef errval_t (*mapping_handler_t)(struct capability *dest_cap, cslot_t dest_slot,
                                      struct capability *src_cap, uintptr_t flags,
...
errval_t caps_copy_to_vnode(struct cte *dest_vnode_cte, cslot_t dest_slot,
                            struct cte *src_cte, uintptr_t flags, uintptr_t offset,
                            uintptr_t pte_count, struct cte *mapping_cte)
{
    ...
    if (err_is_fail(err)) {
        assert(mapping_cte->cap.type == ObjType_Null);
        debug(SUBSYS_PAGING, "caps_copy_to_vnode: handler func returned %" PRIuERRV "\n",
              err);
        return err;
    }
                             ...
                             uintptr_t pages, uintptr_t kpi_paging_flags)
{
    ...
    for (int i = 0; i < pages; i++) {
        union armv8_ttable_entry *entry = (union armv8_ttable_entry *)base + i;
        paging_set_flags(entry, kpi_paging_flags);
    }
    ...
    for (uint16_t l0idx = first_l0idx; l0idx <= last_l0idx; l0idx++) {
        ...
        for (uint16_t l1idx = first_l1idx; l1idx <= last_l1idx; l1idx++) {
            ...
            for (uint16_t l2idx = first_l2idx; l2idx <= last_l2idx; l2idx++) {
                ...
                last_l3idx = l2idx == last_l2idx ? VMSAv8_64_L3_INDEX(end)
                                                 : PTABLE_ENTRIES;
                // read pdpt base
